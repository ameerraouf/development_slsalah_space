                                    ...
                                    @if ($message->file)
                                        <span class="d-block">
                                            <a href="{{ $message->file }}" target="_blank"
                                                class="btn-primary px-2 rounded text-start">المرفق</a>
                                        </span>
                                    @endif
                                    @if ($message->audio)
                                ...
                                <div class="align-items-center mx-1">
                                    ...
                                    @if ($message->file)
                                        <span class="d-block text-end">
                                            <a href="{{ $message->file }}" target="_blank"
                                                class="text-end btn-primary px-2 rounded ">المرفق</a>
                                        </span>
                                    @endif
                                    @if ($message->audio)
    ...
    <script>
            ...
            $('#chat-form').submit(function(e) {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        var messageContainer = $('<div>').addClass(
                            'd-flex align-items-center my-2');
                        var avatarContainer = $('<div>').css({
                            height: '40px',
                            width: '40px'
                        });
                        var avatarImage = $('<img>').addClass('rounded-circle h-100 w-100')
                            .attr('src', "{{ $userPhoto }}");
                        var contentContainer = $('<div>').addClass('align-items-center mx-1');

                        var contentText = $('<span>').addClass('d-block text-start').text(
                            message);
                        var timestampText = $('<span>').addClass('d-block text-start').text(
                            timestamp);

                        $('#chat-messages').append(clearFix);
                        contentContainer.append(contentText);
                        if (data.file != null) {
                            var fileLink = $('<a>').attr('href', data.file).attr('target',
                                    '_blank').addClass('btn-primary px-2 rounded d-block w-50')
                                .text('المرفق');
                        }
                        ...
                        if (data.audio != null) {
                            var audioElement = $('<audio>').attr('controls', '').addClass(
                                'audio-element my-2');
                            var sourceElement = $('<source>').attr('src', data.audio).attr(
                                'type', 'audio/mpeg');
        ...
        function receiver(data) {
            ...
            if (data.message.file != null) {
                var fileLink = $('<a>').attr('href', data.message.file).attr('target', '_blank').addClass(
                    'btn-primary px-2 rounded d-block text-end w-50').text('المرفق').css('margin-right', 'auto');
            }
            ...
            $('#message-file').change(function() {
                ...
                reader.onload = function(e) {
                    ...
                    if (fileExtension === 'pdf') {
                        previewHtml =
                        '<i class="fa fa-file-pdf-o" style="font-size:35px"></i>'; // Replace "fa-file-pdf-o" with the appropriate class for your font awesome icon
                    } else if (fileExtension === 'doc' || fileExtension === 'docx') {
                        previewHtml =
                        '<i class="fa fa-file-word-o"></i>'; // Replace "fa-file-word-o" with the appropriate class for your font awesome icon
                    } else {
                        previewHtml = '<img src="' + e.target.result +
                            '" height="50" width="50" style="margin-bottom:5px">';
                    }
